...
- **マルチ出力フォーマット**: Eww, Waybar などの異なるツールに対応するため、JSON, Waybar, Raw などの形式をサポート。

### Logging & Debugging
- **集約ログ**: デーモン実行時は `/tmp/lyrics-daemon.log` に全ての標準出力・エラー出力を記録。
- **詳細デバッグ**: `stderr` を活用し、結果出力（`stdout`）と分離したログ記録を行う。
